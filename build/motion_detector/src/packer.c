...
PyObject *c_pack(PyObject *py_rects, PyObject *bins)
{
  ...
  for (int i = 0 ; i < nb_rects ; ++i) {
    ...
    if (rect_ordered_add(&c_rects, PyLong_AsLong(PyTuple_GetItem(rect_tuple, 0)),
		    PyLong_AsLong(PyTuple_GetItem(rect_tuple, 1)),
  ...
  for (int i = 0 ; i < nb_bins ; ++i) {
    ...
    int height = PyLong_AsLong(PyTuple_GetItem(bin_tuple, 0));
    int width = PyLong_AsLong(PyTuple_GetItem(bin_tuple, 1));
    int count = PyLong_AsLong(PyTuple_GetItem(bin_tuple, 2));
