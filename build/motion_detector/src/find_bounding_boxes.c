...
PyObject *c_find_bounding_boxes(PyObject *py_rectangles)
{
  ...
  for (int i = PyList_Size(py_rectangles) - 1 ; i >= 0 ; --i) {
    ...
    if (add_box(&rectangles, PyLong_AsLong(PyTuple_GetItem(rect, 0)),
                                PyLong_AsLong(PyTuple_GetItem(rect, 1)),
                                PyLong_AsLong(PyTuple_GetItem(rect, 2)),
                                PyLong_AsLong(PyTuple_GetItem(rect, 3))))
